...
// 9. Gallery Carousel Slider Component Logic
function initGalleryCarousel() {
  ...
  if (dotsContainer) {
    dotsContainer.addEventListener('click', e => {
      ...
      const targetIdx = parseInt(targetDot.getAttribute('data-slide'), 10);
  ...
  // Autoplay function
  const startAutoplay = () => {
    stopAutoplay(); // Stop any existing interval to prevent memory leaks and rapid cycling
    autoplayTimer = setInterval(handleNext, 4000); // 4 seconds
  };
  ...
  // Hover detection to pause autoplay
